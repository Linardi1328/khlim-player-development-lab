...
const schema = z.object({ token: z.string().min(20).max(2000), password: z.string().min(10, "Use at least 10 characters.").max(128) });
...
export async function POST(request: Request) {
  return response(async () => {
    ...
    if (!user?.passwordHash) throw new AppError(400, "This reset link is invalid or expired.");
