...
import { buildCoachDraftPrompt, type CoachDraftTarget } from "../src/lib/coach-ai";
import { createLabResetToken, readLabResetEmail, verifyLabResetToken } from "../src/lib/lab-reset";
...
describe("lab password reset tokens", () => {
  afterEach(() => { delete process.env.LAB_RESET_SECRET; });
  it("binds a short-lived token to the current password hash", () => {
    process.env.LAB_RESET_SECRET = "test-secret";
    const token = createLabResetToken("coach1@playerlab.example.test", "hash-v1");
    expect(readLabResetEmail(token)).toBe("coach1@playerlab.example.test");
    expect(verifyLabResetToken(token, "hash-v1")).toBe("coach1@playerlab.example.test");
    expect(() => verifyLabResetToken(token, "hash-v2")).toThrow("invalid or expired");
  });
});

describe("coach AI drafting prompt", () => {
  const cases: Array<[CoachDraftTarget, Record<string, string>]> = [
    ["focus", { name: "Fictional Athlete", group: "U12", position: "Guard", hidden: "do not include" }],
    ["assessmentNotes", { shooting: "4", finishing: "2", passing: "4", defense: "2" }],
    ["practicePlan", { title: "Improve left-hand finishing", target: "8 of 10 off-hand layups" }],
  ];
  it.each(cases)("builds a constrained %s prompt", (target, context) => {
    ...
    expect(prompt.instructions).toContain("Use only the supplied form context.");
